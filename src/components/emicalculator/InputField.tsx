...
const InputField: React.FC<InputFieldProps> = ({
  ...
  sliderMax,
  sliderStep,
  required = id === "productName" ? false : true,
}) => (
  <motion.div
    initial={{ opacity: 0, y: 10 }}
    animate={{ opacity: 1, y: 0 }}
  ...
  >
    <label
      htmlFor={id}
      className="flex items-center text-base font-medium text-gray-900 dark:text-black-200"
    >
      {icon}
      <span className="ml-1">{label}</span>
      <Info
        className="w-4 h-4 ml-1 hover:text-blue-500 transition-colors"
        aria-hidden="true"
      />
    </label>
    {sliderMin !== undefined &&
      ...
      sliderStep !== undefined && (
        <input
          ...
          value={value || "0"}
          onChange={(e) => onChange(e.target.value)}
          className="hidden md:block w-full h-2 rounded-lg cursor-pointer bg-gray-200 accent-blue-500 hover:accent-blue-600 transition-colors"
          aria-label={`Adjust ${label.toLowerCase()}`}
        />
      )}
    <div className="relative">
      <input
        ...
        className={`block w-full border ${
          error ? "border-rose-500" : "border-blue-500"
        } bg-white dark:bg-gray-800 dark:text-gray-200 text-gray-900 rounded-md p-2 pl-10 text-base focus:ring-2 focus:ring-blue-400 focus:border-blue-600 hover:scale-[1.02] transition-all`}
        aria-describedby={`${id}-error`}
        aria-invalid={!!error}
      />
      {icon && (
